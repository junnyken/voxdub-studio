...
def tim_venv_cu_bat_ky(ten_venvs) -> str:
    ...
    Khác :func:`tim_ban_cai_cu` ở chỗ KHÔNG đòi ``installed_ok.json``: venv
    torch cho Demucs không có marker nào, sự tồn tại của thư mục chính là dấu
    hiệu (đúng như ``gpu_venv_dir()`` vẫn kiểm).
    """
    key = ("__venv__", ",".join(ten_venvs))
    with _lock:
        if key in _cache:
            return _cache[key] or ""
    ...
    for i, entry in enumerate(anh_em):
        ...
        try:
            ...
            continue
        for ten in ten_venvs:
            duong = os.path.join(entry.path, ten)
            if os.path.isfile(_python_trong(duong)):
                ket_qua = duong
                logger.info(f"Dùng lại bộ đã cài của bản trước ở thư mục: "
                            f"{os.path.basename(entry.path)}")
                break
        if ket_qua:
            break
