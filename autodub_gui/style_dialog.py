...
class StyleDialog(QDialog):
    ...
    def __init__(self, video_path: str | None, style: dict,
        ...
        # V5 (docs/PLAN.md): đề xuất sẵn vùng che chữ overlay thay vì bắt
        # người dùng tự vẽ tay từ đầu — chỉ bật khi có video thật (cần
        # trích frame), giữ nguyên khả năng vẽ tay làm phương án chính/
        # dự phòng (guardrail 2 của mini-spec: không bắt buộc OCR).
        if has_video:
            ocr_row = QHBoxLayout()
    ...
    def _start_ocr_scan(self) -> None:
        """V5 (docs/PLAN.md): quét chữ tự động, đề xuất vùng che — người
        dùng vẫn xác nhận/sửa/xoá được như vẽ tay bình thường."""
        from autodub.config import Settings
        from autodub_gui.ui.toast import TOASTS

        self.btn_ocr_scan.setEnabled(False)
        self.btn_ocr_scan.setText("Đang quét...")
        try:
