...
class RecognizeStep(_StepPanel):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        # V4 (docs/PLAN.md): Paraformer chỉ hỗ trợ tiếng Trung — cảnh báo
        # sớm thay vì để người dùng ngỡ ngàng khi backend tự âm thầm dùng
        # Whisper (transcriber.transcribe() đã fallback an toàn, đây chỉ là
        # thông báo, không chặn).
        self.paraformer_warning = QLabel(
            "Paraformer chỉ hỗ trợ tiếng Trung — ứng dụng sẽ tự dùng "
            "Whisper cho ngôn ngữ bạn vừa chọn.")
        self.paraformer_warning.setWordWrap(True)
        self.paraformer_warning.setStyleSheet(
            f"color: {tokens.WARNING}; font-size: {tokens.FS_META}px; "
            f"background: transparent;")
        self.paraformer_warning.setVisible(False)

        for widget in (self.engine, self.model, self.language):
            widget.changed.connect(self.changed.emit)
        self.engine.changed.connect(self._update_paraformer_warning)
        self.language.changed.connect(self._update_paraformer_warning)
        self.body.addWidget(self.engine)
        self.body.addWidget(self.model)
        self.body.addWidget(self.language)
        self.body.addWidget(self.paraformer_warning)
        self.body.addWidget(self.auto_detect)
        self._update_paraformer_warning()
    ...
    def _update_paraformer_warning(self) -> None:
        mismatch = consts.paraformer_language_mismatch(
            self.engine.current_key(), self.language.current_key())
        self.paraformer_warning.setVisible(mismatch)
