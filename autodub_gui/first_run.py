...
def translate_mode_check() -> tuple[str, str, str, object]:
    """Trạng thái dịch tự động — phân biệt rõ CÓ máy chủ vs KHÔNG có.

    Trước đây mục này chỉ đọc ``settings.translate_enabled`` (một công tắc
    bật/tắt trong Cài đặt) mà bỏ qua ``saas_client.is_configured()`` (có
    ``VOXDUB_API_URL`` hay không) — nên hiện "đã sẵn sàng — chạy qua máy
    chủ VoxDub" ngay cả khi KHÔNG hề có máy chủ nào được cấu hình (trường hợp
    phổ biến nhất khi tự chạy từ mã nguồn: mặc định dịch chuyển sang dịch tay,
    không tự động, không tốn Vox). Sửa theo mini-spec V3 (docs/PLAN.md).
    """
    ...
    if is_configured():
        return (
            "Dịch tự động",
            "Chạy qua máy chủ đã cấu hình — máy mới được tặng Vox dùng thử; "
            "xem số dư ở trang Tài khoản. Tính năng này tốn Vox, phần lồng "
            "tiếng chính (nghe-chép, đọc giọng Việt, ghép video) vẫn miễn "
            "phí và chạy trên máy bạn.",
            "", lambda s: bool(s.translate_enabled),
        )
    return (
        "Dịch tự động",
        "Chưa kết nối máy chủ nào — mặc định dịch TAY (ứng dụng ghi sẵn lời "
        "nhắc, bạn dán vào ChatGPT/Gemini). Toàn bộ ứng dụng vẫn chạy 100% "
        "miễn phí, offline. Muốn dịch tự động thì tự dựng máy chủ (xem mục "
        "\"Dành cho lập trình viên\" trong README) rồi đặt VOXDUB_API_URL.",
        "", lambda s: False,
    )


def _all_checks():
    return (*_STATIC_CHECKS, translate_mode_check())


def mode_banner_text() -> str:
...
class FirstRunDialog(QDialog):
    ...
    def __init__(self, settings, parent: QWidget | None = None):
        ...
        mode = QLabel(mode_banner_text())
        mode.setWordWrap(True)
        mode.setStyleSheet(
            f"background: {tokens.BG_SELECTED_SOFT}; border-radius: 8px; "
            f"padding: {tokens.SP_2}px {tokens.SP_3}px; color: "
            f"{tokens.TEXT_PRIMARY}; font-size: {tokens.FS_LABEL}px; "
            f"font-weight: 600;")
        root.addWidget(mode)

        for name, description, command, probe in _all_checks():
