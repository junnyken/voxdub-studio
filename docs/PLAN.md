...
| V32a | PoC lip-sync MuseTalk — benchmark thật + thử nghiệm consent-check/watermark (Phase G, đóng gap V30) | 🔶 Hạ tầng cài đặt + harness đo THẬT đã viết xong, **CHẶN THẬT: sandbox không có GPU** | Chủ dự án đã trả lời đủ 5 câu hỏi chính sách V30 (2026-08-12): CÓ consent-check, CÓ watermark, CÓ giới hạn theo gói, CHẤP NHẬN GPU-only, KHÔNG cần Wav2Lip. `scripts/setup_lipsync_poc.py` (cài `.venv-lipsync`, clone MuseTalk ghim commit `0a89dec4`, tải weights thật ~5-6GB — dựa đúng README/`requirements.txt`/`download_weights.sh` chính chủ, đã xác nhận qua GitHub API, không suy đoán) + `scripts/research/lipsync_poc.py` (harness đo Scope B/C/D: benchmark thời gian/VRAM qua `scripts/inference.py` thật của MuseTalk, consent-check gọi thẳng `musetalk.utils.preprocessing.get_landmark_and_bbox()` thật, 2 phương án watermark ffmpeg). CHƯA chạy được ở đây — cần máy có GPU NVIDIA thật (script tự gate chặn ở bước đầu nếu thiếu, không phí cài ~15GB); chủ dự án tự chạy trên máy có GPU riêng — xem TEST_LOG |
