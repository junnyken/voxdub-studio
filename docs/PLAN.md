...
| V54 | `cloud-batch` nhận cả liên kết, không chỉ file trên máy (Phase G, 2026-08-17) | ✅ Xong, CHƯA chạy thật | Liên kết được tải về máy trước (tái dùng `download_one` — yt-dlp + đường Douyin riêng) rồi mới đẩy lên; khoá trạng thái là CHÍNH liên kết nên tên file tải về đổi cũng không nộp trùng. Bản tải trung gian **xoá sau khi thành công, GIỮ khi hỏng** để lượt sau khỏi tải lại vài trăm MB. Gỡ luôn giới hạn "phải cùng thư mục" của V53. CLI thêm `--file` (danh sách, hỗ trợ cú pháp «link \| Tên giọng»). 6 test mới **18-08**: đường file trên máy đã chạy thật; đường LIÊN KẾT (yt-dlp/Douyin) vẫn chưa chạy thật. |
