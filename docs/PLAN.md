...
| V69 | Key Gemini riêng cho VoxDub + vá lỗ ghi nhật ký token (Phase H, 2026-08-18) | ✅ Xong + 3 test, ĐÃ áp lên prod | Key riêng gắn qua `PATCH /v1/admin/providers/:id`, verify bằng lượt dịch thật QUA máy chủ. Lúc verify lộ lỗ: `UsageLog` chỉ ghi ở đường app desktop, đường **API key** đốt token thật mà chỉ tăng `usageCount` (đếm LƯỢT, không phải token) — đối soát hoá đơn bằng nó là đối soát với con số không liên quan. Đã ghi log cho cả đường API key, `fingerprint = apikey:<prefix>` để truy được key nào tiêu tiền. |
| V70 | Người nói quá ít để tính đặc trưng giọng — nói thẳng thay vì im lặng (Phase H, 2026-08-18) | ✅ Xong + 3 test | Đo trên giọng người thật lộ ra pyannote trả NaN/toàn-0 cho người nói 3.1 giây. Tầng trên vốn đã an toàn (coi như không khớp) nhưng IM LẶNG — người dùng thấy "nhân vật mới" mọi tập mà không hiểu vì sao. Worker bỏ vector đó kèm cảnh báo có cách sửa. |
| V71 | Chép lời: giọng nói thành văn bản, từ liên kết/video/mp3 (Phase H, 2026-08-18, chủ dự án yêu cầu) | ✅ Xong + 29 test, chạy thật trên clip thật | Audit trước khi build cho thấy MỌI mảnh nặng đã có sẵn (`download_one`, `extract_audio`, `transcribe`, `generate_srt`) — V71 chỉ là lớp nối mỏng + bản xuất `.txt`/`.vtt`. Module RIÊNG chứ không phải cờ `--skip-*` cắm vào pipeline: dub luôn chạy tiếp sang dịch+TTS+ghép video, ai chỉ cần chữ thì mọi bước sau ASR là thời gian bỏ đi. CLI `voxdub transcribe` + trang "Chép lời" chạy trong QThread. **3 bug do test bắt**: VTT `00:00:60.000` (không có giây thứ 60 — trình phát từ chối cả file), tên file toàn gạch dưới khi tiêu đề chỉ có ký tự cấm, và `LabeledCombo` viết ngược `(khoá, nhãn)`. Chạy thật: 20 câu, 4 định dạng, SRT dùng dấu phẩy còn VTT dùng dấu chấm — đúng chuẩn. |
| V72 | Dừng giữa chừng + chép lời hàng loạt (Phase H, 2026-08-18) | ✅ Xong + 20 test, verify bằng chạy thật | **Nút Dừng phải sửa HAI lần, cả hai do chạy thật bắt được**: bản đầu kẹt trong `readline()` nên không bao giờ tới chỗ kiểm cờ (sửa: luồng canh giết tiến trình); bản hai vẫn treo vì máy không có `.venv-whisper` nên ASR chạy đường IN-PROCESS — chỗ chưa hề cài huỷ (lỗi sửa một trong hai đường đi). Sau khi sửa: thoát sau 10 giây, mã thoát 0, không sinh file dở. Hàng loạt: `--input` lặp được + nhận thư mục, chạy tuần tự, một mục hỏng không làm hỏng cả mẻ, hai file trùng tên không ghi đè nhau. |
| V72b | Đóng 2 gap của V72 (Phase H, 2026-08-18) | ✅ Xong + 4 test | Liên kết trùng tiêu đề giờ cũng được thêm hậu tố (tên liên kết chỉ biết sau khi tải, nên chống trùng phải làm SAU chứ không trước). Và đường subprocess từ "tin là được" thành **đo được**: test giả lập worker `sleep(600)` chứng minh huỷ xong trong ~1,5 giây. Độ trễ huỷ KHÁC nhau theo đường chạy: subprocess gần như ngay, in-process phải chờ hết câu đang nhận dạng (~10s). |
| V73 | 2 lỗi người dùng báo trên bản `.exe` v3.4.0 (Phase H, 2026-08-18) | ✅ Xong + 30 test | **Không phải lỗi tự tìm ra — người dùng cài bản phát hành, dán một liên kết YouTube, chụp màn hình gửi lại.** (1) Liên kết sao chép lúc đang nghe Mix (`watch?v=…&list=RDMM…`) hỏng với MỌI video: thiếu `noplaylist` nên yt-dlp coi là danh sách phát — đo thật, liên kết đó trả về **194 mục**, tải hết rồi mới hỏng vì `_resolve_filepath` đi tìm một file theo id của playlist. `grep playlist` cả repo trước khi sửa = **0 kết quả**, nên lỗi đánh vào cả 4 đường dùng liên kết chứ không riêng Chép lời. (2) Dòng «HỎNG» không có lý do: `BatchItem.error` bị vứt hai lần độc lập — tín hiệu `item_status` của `TranscribeWorker` khai thiếu tham số (worker DUY NHẤT trong workers.py thiếu, `DownloadWorker`/`BatchWorker` đều đã có), còn cảnh báo của lõi thì bị `log_text.notice_for` lọc vì có chứa URL. (3) Thấy thêm trong cùng ảnh: thanh bên **chồng mục lên nhau** — «Chép lời» đè nhãn HỆ THỐNG 44px ở màn hình 1080p. Ba danh sách khoá cứng chiều cao nên layout hết chỗ co; thanh bên đòi tối thiểu 1055px trong khi vùng làm việc chỉ ~1000px. `_sync_footer` (viết ra để chống đúng lỗi này) **không giải phóng được pixel nào** — đo được `sizeHint` giữ nguyên 1055 sau khi ẩn cả hai thẻ. Sửa bằng `QScrollArea`: tối thiểu rơi 1055px → **172px**; ở 1000px mọi mục hiện đủ mà không phải cuộn. **Soát lại lộ thêm 3 lỗi do CHÍNH bản sửa gây ra** (tự tìm, chưa ai gặp): hàm chặn playlist chặn nhầm `youtu.be/<id>?list=…` — id video nằm ở ĐƯỜNG DẪN chứ không phải tham số `v=`, mà đó đúng là dạng nút Chia sẻ YouTube sinh ra, tức tệ hơn lỗi gốc; `noplaylist` một mình KHÔNG đủ vì `shorts/<id>?list=` vẫn hỏng (yt-dlp chọn extractor `youtube:tab` trước cả khi `noplaylist` có tiếng nói) nên `normalize_url` phải cắt hẳn `list`/`start_radio`/`index`; và vùng cuộn mới làm lăn chuột trên menu mất tác dụng (`QListWidget` nuốt bánh xe kể cả khi tự nó không cuộn được). **Đợt 3 đóng nốt 2 việc tồn**: `_don_tam` dọn file tạm sau khi xuất xong — ràng buộc sống còn là CHỈ xoá thứ trong `work_dir`, vì với file trên máy `prepare_audio` trả về chính đường dẫn của NGƯỜI DÙNG; và `looks_like_bare_url` nhận liên kết thiếu `https://` (cố tình hẹp — `phim.mp4` trông y hệt tên miền nếu chỉ soi đuôi). **Giới hạn verify**: phân giải liên kết kiểm được bằng yt-dlp thật, nhưng KHÔNG tải thật được từ sandbox (403 Forbidden do IP — video 19 giây không dính playlist cũng 403). 51 test mới, 1426 passed. |
| V74 | "Đã cài Whisper rồi mà app vẫn bảo chưa" (Phase H, 2026-08-19, người dùng báo) | ✅ Xong + 6 test | Một gốc rễ, ba biểu hiện: `autodub.spec` cố ý loại `faster_whisper`/`ctranslate2`/`av` khỏi bundle (Whisper chạy trong `.venv-whisper`), nhưng (1) `preflight._check_asr` vẫn kiểm bằng `import faster_whisper` → hộp thoại chặn hiện ở MỌI lần mở app với 100% người dùng bản đóng gói, kể cả người đã cài đúng; (2) `transcribe()` rơi sang in-process — đường không tồn tại trong `.exe` — biến tình huống nói được thành `No module named 'av'`; (3) `TranscribeCancelled` kế thừa `RuntimeError` nên `except Exception` nuốt gọn, bấm Dừng xong máy CHẠY LẠI TOÀN BỘ ở in-process (V72 không bắt được vì máy thử nghiệm không có `.venv-whisper`). Đúng lớp lỗi đã sửa ở `_smoke_report` hồi V38 — lần đó chưa rà các chỗ khác cùng giả định. **Nguyên nhân trực tiếp**: `.venv-whisper` nằm TRONG thư mục ứng dụng, nên nâng cấp lên thư mục mới là mất sạch. **Dòng lỗi có lý do của V73 chính là thứ làm ca này chẩn đoán được.** Còn tồn: phần Python của faster-whisper vẫn nằm trong bundle dù spec định loại (`collect_all` thêm lại qua `hiddenimports`, `_ML_PRUNE` chỉ lọc binaries) — **Đã mở bản phát hành ra đếm — con số bác bỏ phán đoán đó**: phần thừa là **~1,8 MB chứ không phải 112 MB** (`_keep_binary` đã cắt DLL native từ lâu, `av` đã sạch 0 tệp). Nhưng 1,8 MB mã Python đó đủ gây hại vì `_internal/` nằm trên `sys.path` của bản onedir. Nguyên nhân `excludes` không chặn được: nó chỉ tác động ở TẦNG IMPORT, còn `collect_all` nhét tệp vào qua TẦNG DATAS — hai dòng mâu thuẫn trong cùng một tệp spec. Đã bỏ khỏi `collect_all`, thêm test đọc thẳng spec chặn tái diễn. **Lỗi thứ 4 (bản sửa V74 đầu chưa che)**: `BatchWorker` tạo `WhisperCache()` khi mẻ có ≥2 video, mà điều kiện cũ đòi `whisper_cache is None` — nên Xử lý hàng loạt ≥2 video bỏ qua hẳn `.venv-whisper`, hỏng trên mọi bản đóng gói trong khi chạy LẺ 1 video vẫn tốt. Quét AST toàn repo đối chiếu `excludes`: 6 chỗ import gói bị loại, 5 vô hại (probe, đường dự phòng có bọc lỗi, nhánh chỉ chạy bản mã nguồn). 14 test, 1440 passed. |
| V75 | Canh chữ karaoke chạy được ở bản đóng gói (Phase H, 2026-08-19, tự soi ra từ bảng rà V74) | ✅ Xong + 14 test | Lỗi thứ 6 cùng gốc rễ với V38/V74, khác ở chỗ **nó chưa từng kêu một tiếng**: `align.py` import `faster_whisper` ngay trong tiến trình chính, mà `autodub.spec` cố ý không đóng gói gói đó, còn call site trong `ass_karaoke.resolve_word_times` thì bọc `try/except` → ở bản `.exe` **100% lượt canh chữ đều hỏng** và phụ đề âm thầm rơi về chia đều theo thời lượng câu. Không log nào tới người dùng, nên không ai báo — bảng rà V74 xếp nó là "suy giảm âm thầm" và để lại. **Sửa**: thêm `align_whisper_worker.py` chạy trong `.venv-whisper` — nạp model `base` MỘT LẦN cho cả mẻ rồi nghe hàng trăm clip ngắn (single-shot như `asr_whisper_worker.py` sẽ phải nạp lại model mỗi clip), một clip hỏng chỉ mất câu đó. Thứ tự chọn đường giống hệt `transcribe()` để không đẻ ra quy tắc thứ hai. `settings` phải thread từ `resolve_word_times` xuống, nếu không `align_segments` không biết venv nằm đâu. **Chỗ suýt vẫn im lặng**: `log_text.notice_for` là allowlist và `_TECH_RE` loại mọi câu chứa chữ "whisper" → thông báo mới bị lọc sạch nếu không thêm dòng riêng vào `NOTICES` (bài học V73). **Đã kiểm bằng faster-whisper THẬT** (venv riêng + model base 142 MB tải thật + giọng người thật): 2 clip canh xong trong 4,0s, mốc chữ ra thật và mốc karaoke cuối cùng KHÔNG còn đều nhau. 1454 passed. |
| V77 | Nâng cấp không còn mất bộ nghe đã cài (Phase H, 2026-08-19) | ✅ Xong + 10 test | `.venv-whisper`/`.venv-asr`/`.venv-vieneu` và `models/` nằm TRONG thư mục ứng dụng, nên cách nâng cấp duy nhất (giải nén bản mới ra thư mục khác) làm app báo "chưa cài" dù đã cài từ lâu — chữa tay là chép 2 thư mục hoặc tải lại ~1,5 GB. Đây là cạm bẫy đã ghi trong V74 nhưng để ngỏ. Giờ khi thư mục mặc định trống, app dò các thư mục nằm CẠNH nó (chính là các bản cũ) và dùng lại tại chỗ. Ba ràng buộc: không đè cài đặt tay, không nhận bản cài dở (thiếu `installed_ok.json`), và có nhớ đệm + trần 60 thư mục (app có thể nằm trong Downloads hàng trăm thư mục). Dòng "đang dùng lại bộ của bản cũ" có lời riêng để hiện lên Nhật ký. |
| V78 | Suy giảm âm thầm phải nói ra (Phase H, 2026-08-19) | ✅ Xong + 28 test | Quét AST toàn bộ `logger.warning/error` của lõi rồi cho chạy qua chính `log_text.notice_for`: **43 dòng không bao giờ tới được khung Nhật ký**. Phần lớn đúng là chi tiết kỹ thuật, nhưng 10 dòng thì người dùng lãnh hậu quả thật mà không được báo: Demucs hỏng → video mất hẳn nhạc nền; thiếu bộ chấm câu tiếng Trung → transcript không dấu câu; Paraformer đã chọn nhưng bị bỏ qua; atempo lỗi → tốc độ giọng không áp được; không ghi được sổ dịch tạm → lượt chạy lại phải TRẢ TIỀN dịch lần nữa. Đã soạn lời thường cho từng ca. Còn 28 dòng cố ý vẫn ẩn; riêng "Rà soát bản dịch lỗi" giữ ẩn theo quyết định có sẵn từ trước, ghi thành test để lần sau ai đổi thì đổi có ý thức. |
| V79 | Nút Dừng cắt ngang được các bước dài nhất (Phase H, 2026-08-19) | ✅ Xong + 10 test | Rà tiếp sau V76: `rep.check_cancelled()` chỉ nằm GIỮA các bước (12 chỗ), nên bấm Dừng lúc đang tách nhạc nền (Demucs, 10+ phút), xuất video (ffmpeg re-encode), dịch máy hay đồng bộ khẩu hình đều vô tác dụng. Thêm `autodub/cancel_guard.py` (giết tiến trình con + đổi lỗi hậu-quả-của-giết thành `PipelineCancelled`) và nối cờ từ pipeline xuống 4 bước. **Bẫy riêng của nhóm này**: mỗi bước có sẵn một đường DỰ PHÒNG âm thầm — Demucs "hỏng" thì video ra KHÔNG CÓ NHẠC NỀN, dịch máy "hỏng" thì rơi sang dịch tay. Nên phải chặn `kiem_dung()` ở cả `except Exception` của `separate_vocals`. Xuất video giữ nguyên `subprocess.run` khi không có cờ Dừng — không đổi cách chạy bước quan trọng nhất chỉ vì tính năng không dùng tới ở đó. |
| V82 | Thiếu FFmpeg phải nói ra ở đúng chỗ người dùng đứng (Phase H, 2026-08-19, người dùng báo) | ✅ Xong + 7 test | Hai dòng lỗi thật: `[WinError 2] The system cannot find the file specified` (file trên máy) và `ERROR: ... ffmpeg is not installed` (yt-dlp). Cùng một nguyên nhân nhưng không ai đoán được. Thêm `autodub/ffmpeg_deps.py` (một chỗ duy nhất trả lời "máy có FFmpeg chưa"), chặn sớm trong `transcribe_media` với lời rõ ràng, dịch cả hai câu lỗi trong `FRIENDLY_ERRORS`, và trang Chép lời in lời đã soạn thay vì nguyên văn. Thêm **`Cai dat FFmpeg (bat buoc).bat`** — FFmpeg là thành phần bắt buộc nhưng lại là thứ DUY NHẤT không có tệp .bat để đúp chuột, trong khi Whisper/VieNeu/Paraformer/Douyin đều có. 1548 passed. |
| V84 | Link tải bộ giọng mẫu trỏ sang kho không tồn tại (Phase H, 2026-08-19) | ✅ Xong + 6 test | `VOICES_RELEASE_URL` ghim `ttthanh2044/voxdub` trong khi bản phát hành nằm ở `junnyken/voxdub-studio`. **Đo bằng HTTP: URL cũ 404, URL mới 302** ⇒ tính năng "Nạp bộ giọng đọc mẫu" (120 giọng) chưa bao giờ tải được trên bản đóng gói. Cùng lớp với V80 (worker không được đóng gói) và V83 (`icons.brand_logo` không tồn tại): thứ được gọi tới thì có, thứ ở đầu bên kia thì không. Chữa bằng cách bỏ hằng số ghim tay — đọc `Settings.update_repo`, một nguồn sự thật với việc kiểm tra cập nhật. Test quét toàn repo chặn kho chết tái xuất; lòi thêm `chay_app.bat` cũng còn trỏ về kho cũ. |
| V86 | "Không tách được nhạc nền" trên MỌI bản đóng gói (Phase H, 2026-08-19, người dùng báo) | ✅ Xong + 6 test | `autodub.spec` cố ý loại torch/demucs/soundfile (nặng hàng GB) — Demucs chạy trong `.venv-gpu` qua `demucs_worker.py`. Nhưng **chưa từng có script nào tạo venv đó**: 11 tệp `scripts/setup_*.py` không tệp nào cho Demucs, tài liệu chỉ nhắc tên `.venv-gpu` như thể nó tự có. Nên bước Tách nhạc nền luôn hỏng và video ra chỉ còn giọng đọc. Cùng hình dạng V80/V83/V84: **đường dẫn thì có, thứ ở đầu kia thì không.** Thêm `scripts/setup_demucs.py` (torch CUDA nếu có card NVIDIA, không thì CPU ~200 MB) + `Cai dat tach nhac nen (Demucs).bat`, thông báo lỗi chỉ đúng tệp cần bấm, và `.venv-gpu` cũng mượn được của bản cũ (nặng ~2,5 GB, đừng bắt tải lại sau mỗi lần nâng cấp). 1567 passed. |
| V95 | Đóng nốt hai mục "còn tồn" tự ghi (Phase H, 2026-08-20) | ✅ Xong + 4 test | Quét lại mọi mục "Còn tồn" từ V75→V94 thay vì trả lời theo trí nhớ. **Sửa**: `editor.py` nay chuyển tiếp cờ Dừng xuống `refresh_subtitles`/`merge_video` (hai worker dựng lại ĐÃ có nút Dừng, cờ chỉ dừng ở giữa đường) — thêm `ProgressReporter.cancel_event` để khỏi chọc vào thuộc tính riêng. **Ghi rõ là quyết định, không phải bỏ sót**: OCR vùng chữ không nối cờ vì hộp thoại gọi nó không có nút Dừng, thêm tham số chỉ là mã chết — test khoá hai chiều, ngày nào có nút Dừng thì đỏ. Còn lại 5 mục đều là giới hạn thật hoặc mở rộng tính năng. Ghi nhận một lần core dump lúc dọn dẹp không tái hiện được sau 4 lượt chạy lại. 1638 passed. |
| V94 | Vai `assist` không lưu được — việc "2 phút" là bất khả thi (Phase H, 2026-08-20) | ✅ Xong + 4 test | V89 thêm cổng trợ lý gọi `providersFor('assist')` nhưng `AiProvider.role` vẫn chỉ nhận `['translate', 'content']` — Mongoose chặn lúc lưu, schema route chặn, form web không có lựa chọn. **Suốt nhiều lượt tôi bảo chủ dự án "thêm một dòng vai assist" trong khi việc đó không làm được.** Không triệu chứng vì đã có đường lui dùng chung vai `translate` — chính đường lui viết ra cho êm đã che mất lỗi. Vai trò bị liệt kê tay ở BỐN nơi độc lập (model, schema route, `<option>`, nhãn bảng) — đúng hình dạng đã cắn hai lượt deploy worker cùng ngày. Test nối bốn nơi: quét mã tìm vai đang được hỏi, bắt model nhận đủ, route không chặt hơn model, và form phải có lựa chọn cho mọi vai. 370 pass (Node), 1634 (Python). |
| V93 | Trả hết nợ 48 chỗ nuốt lỗi im lặng (Phase H, 2026-08-20) | ✅ Xong, luật gọn còn 1 câu | Đọc từng chỗ thay vì đoán. Hai phát hiện đổi hẳn cách làm: (1) bắt lỗi rồi GIỮ nội dung lỗi để hiện ra thì không phải im lặng — bộ dò bản đầu kể oan 5 chỗ, mà bộ kiểm kể oan thì bị tắt; (2) **hầu hết đã có sẵn lời giải thích cạnh dòng `except`**, và lý do nằm cạnh mã tốt hơn hẳn danh sách trong tệp test. Sau khi bộ dò công nhận hai dạng đó: 48 → **2 chỗ** thật sự không giải thích, cả hai đều làm người dùng mất thứ gì đó (trang Báo cáo trống trông y hệt "chưa có dự án nào"; đếm nhầm 0 giọng đã nạp) → thêm log. 28 chỗ còn lại viết lý do ngay cạnh mã. **Bỏ hẳn hai danh sách tập trung**; luật còn một câu: dấu vết, hoặc lý do tại chỗ. 1634 passed. |
| V92 | Nuốt lỗi im lặng: từ "đã xem thấy ổn" thành luật chạy được (Phase H, 2026-08-20) | ✅ Xong + 4 test | Chấm lại 39 chỗ nuốt lỗi kèm TÊN HÀM chứa: ca nguy hiểm nhất là `PreflightWorker.run()` — preflight sập thì `results = []` nên app **không hiện cảnh báo nào** về máy thiếu thành phần, đúng thứ đã làm người dùng mất mấy hôm vì FFmpeg. Thêm dấu vết cho 4 chỗ. Rồi biến đánh giá thành LUẬT: mỗi chỗ nuốt lỗi phải có dấu vết, hoặc nằm trong `DUOC_IM_LANG` kèm lý do viết ra, hoặc trong `CHUA_RA` (mã cũ **chưa ai đọc** — 48 chỗ, đóng băng, chỉ được ngắn đi). Ba test: chỗ mới không thuộc nhóm nào → đỏ; danh sách khai báo không được có dòng thừa; nợ cũ không được phình. 1637 passed. |
| V91 | Rà cuối bằng phân tích tĩnh — tìm ra GỐC RỄ thật của V83 (Phase H, 2026-08-20) | ✅ Xong + 3 test | Chạy `pyflakes` toàn repo: **thân hàm `brand_logo` vẫn nằm trong `icons.py` nhưng MẤT DÒNG `def`** — nó thành mã chết ngay sau `return` của `eye_off()`, còn `icons.brand_logo` thì không tồn tại. Đó mới là gốc rễ thật của V83 (trình cài đặt tự động chưa từng chạy được); hôm 19-08 tôi vá bằng cách viết hàm MỚI ở chỗ khác mà không thấy thân hàm gốc vẫn nằm đó — repo có hai bản vẽ thương hiệu, một bản chết. Nay trả dòng `def` về cho bản gốc và bỏ bản vá. **Chốt chặn thường trực**: test chạy pyflakes chặn `undefined name` + test quét AST bắt "chuỗi kiểu docstring nằm ngay sau `return`" (dấu hiệu hàm mất dòng `def`) — gỡ bản sửa ra thì đỏ đúng ca thật. Kèm sửa nhánh nuốt lỗi im lặng khi kích hoạt mã (V91), dọn 4 cảnh báo nhiễu để tín hiệu thật không bị che. 1633 passed. |
| V90 | Chốt chặn cho bẫy "nhánh deploy không theo main" (Phase H, 2026-08-20) | ✅ Xong + 10 test | Bẫy sập **hai lần** (18-08, 19-08) — lần hai xảy ra DÙ runbook đã có hẳn một mục cảnh báo. Kết luận: tài liệu không sửa được lỗi con người, phải chuyển việc nhắc sang máy. Ba công cụ + một job CI: `kiem_nhanh_deploy.py` (so hash cây thư mục main ⇄ nhánh deploy), `deploy_vays.sh` (MỘT lệnh tự sinh lại nhánh rồi tự kiểm; chặn khi đang ở nhánh khác hoặc còn thay đổi chưa commit), `kiem_deploy_song.py` (so cửa MỚI với cửa CŨ sau khi deploy — chỉ thử cửa mới thì 404 dễ hiểu nhầm thành route sai), và job `deploy-branch-drift` chạy trên mỗi push main. **Test canh chính bộ canh**: đọc `cp -r` trong hai script sinh nhánh rồi bắt ánh xạ của bộ dò phải phủ hết — bản đầu đã kêu nhầm 99 tệp cho worker, mà bộ kiểm hay kêu nhầm thì người ta tắt đi. **Chạy lần đầu phát hiện worker cloud tụt lại ~30 tệp** (thiếu cả `cancel_guard.py` của V79) — đã sinh lại, chờ redeploy. 1629 passed. |
| V89d | Cổng trợ lý — hoàn thiện: nhớ đệm, phiên bản prompt, tên nhân vật (Phase H, 2026-08-19) | ✅ Xong + 13 test | **Nhớ đệm theo NỘI DUNG**: `jobId` chỉ chống gọi trùng do mạng chập chờn, còn người dùng bấm lại nút thì jobId mới → trả tiền lần nữa cho câu hỏi y hệt. Khoá băm (tác vụ + phiên bản prompt + dữ liệu vào, khoá đã sắp nên `{a,b}` = `{b,a}`), lượt dùng lại tính 0 Vox và hiện thành cột riêng ở bảng theo dõi. **Phiên bản prompt** ghi vào nhật ký và nằm trong khoá nhớ đệm — sửa câu chữ là nhớ đệm tự hết hiệu lực, và bảng theo dõi tách được chất lượng trước/sau. **Vai trò thật** cũng được ghi: còn lượt nào chạy bằng vai `translate` nghĩa là chưa cấu hình vai `assist`, trang quản trị hiện cảnh báo đỏ vì đắt hơn hàng chục lần mà không ai để ý. **`character_name` nối xong**: nút "Gợi ý tên gọi" trong hộp thoại Xem trước người nói — lấy lời của CHÍNH người đó từ transcript (câu mẫu một dòng không đủ), hiện kèm lý do, và **không tự ghi vào hồ sơ** vì hồ sơ áp cho mọi tập sau. 1619 passed (Python), 366 (Node). |
| V89c | Cổng trợ lý — giai đoạn 3: bảng theo dõi chi phí (Phase H, 2026-08-19) | ✅ Xong + 10 test | `GET /v1/admin/analytics/assist` + trang **Cổng trợ lý** trong khu quản trị. `analytics/usage` sẵn có gộp theo `action`, mà mọi lượt trợ lý đều mang action `assist` nên không tách được việc nào tốn nhất — cửa mới gộp theo `assistTask`, theo mô hình, và theo mã lỗi. Trả lời đúng ba câu đã hứa: tốn bao nhiêu, việc nào tốn nhất, mô hình nào hay hỏng. Phần dựng truy vấn tách sang `assist-stats.service.js` để **test được không cần cơ sở dữ liệu** — truy vấn gộp sai thì không kêu lên, nó vẫn trả về số, chỉ là số sai. **Test bắt lỗi thật trong mã vừa viết**: `Number(days) || 7` làm `days=0` thành 7 ngày trong khi dòng `Math.max(1, …)` ngay dưới nói là kẹp về 1 — mã và ý định lệch nhau. Cảnh báo tự hiện khi tỷ lệ hỏng ≥5%, kèm cách đọc: hỏng dồn một nơi gọi thì đổi ưu tiên, hỏng đều khắp thì là prompt. 1614 passed (Python), 358 (Node), website build sạch. |
| V89b | Cổng trợ lý — giai đoạn 2: 4 tác vụ còn lại + bộ đo (Phase H, 2026-08-19) | ✅ Xong + 21 test | Thêm `video_summary`, `character_name`, `series_glossary`, `tighten_line` (giá 2 Vox tác vụ ngắn, 5 Vox tác vụ đọc cả transcript). **Bộ đo** `control_server/evals/`: chấm theo TÍNH CHẤT kiểm được bằng máy (câu rút gọn phải ngắn hơn và giữ nguyên con số; từ khoá phải CÓ trong lời thoại; lời giải thích lỗi không được chứa từ kỹ thuật) thay vì "đáp án mẫu" do người viết — đáp án mẫu đòi người đọc từng lượt nên chạy vài lần là bỏ. Chạy khô `npm run eval:assist` (không cần mô hình, chặn lỗi cấu hình) hoặc `--live` (gọi thật, ngưỡng 85%). **Bộ đo tự chứng minh nó bắt được kết quả kém**: 7 test cho ăn kết quả giả tốt/xấu rồi bắt nó chấm đúng — bộ đo luôn xanh còn tệ hơn không có. Ba chỗ bấm thật: nút "Tóm tắt video" (trang Chép lời), nút rút gọn trên TỪNG DÒNG câu (Trình chỉnh sửa), nút "Gợi ý quy ước dịch" (khung ngữ cảnh). `character_name` server-ready nhưng CHƯA nối — cần lời thoại theo từng người nói mà chưa trang nào giữ. 1614 passed (Python), 348 (Node). |
| V89 | Cổng trợ lý đa tác vụ — giai đoạn 1 (Phase H, 2026-08-19) | ✅ Xong + 22 test | Một cửa `/v1/ai/assist` nhận **tên tác vụ**, không nhận prompt: prompt nằm ở `control_server/src/prompts/assist.js` nên sửa câu chữ hay đổi mô hình **không cần phát hành lại .exe**. Tái dùng nguyên hạ tầng sẵn có: `callWithFallback` (tự rơi sang nhà cung cấp khác), `replay`/`remember` (chống gọi trùng), `precheck`/`charge` (ví + hold), `UsageLog`. **Bốn lớp chặn chi phí**: danh sách tác vụ đóng, trần ký tự cắt TRƯỚC khi gọi, hạn mức theo ngày mỗi máy (**mới — server trước đó chỉ có giới hạn theo phút**), giá Vox theo từng tác vụ. Hai tác vụ đầu: `explain_error` (0 Vox, chạy cả khi hết Vox) và `music_suggest` (2 Vox). **Tầng luật V88 không bị thay thế** — máy chủ hỏng/chưa cấu hình/hết Vox đều rơi về nó, và giao diện nói rõ gợi ý đến từ đâu. Mô hình trả sai khuôn hoặc thiếu `reason` bị tính là LỖI, không im lặng đưa kết quả rỗng. Test cũ `hold.test.js` chặn đúng như thiết kế: thêm khoá giá công khai phải cố ý cập nhật danh sách. 1600 passed (Python), 341 (Node). |
| V88 | Gợi ý mô tả nhạc nền suy từ lời thoại (Phase H, 2026-08-19, người dùng đặt hàng) | ✅ Xong + 18 test | V37 sinh được nhạc AI nhưng bắt người dùng tự nghĩ ra mô tả — mà nghĩ ra mô tả tốt mới là phần khó. Nay có nút **"Gợi ý từ nội dung video"** ở Trình chỉnh sửa: đọc transcript đã có, đề xuất tối đa 3 mô tả, bấm một cái là điền sẵn. **Cố ý không dùng AI**: máy chủ chỉ có 4 endpoint cố định (dịch, viết bài, nhạc, SFX), không có đường hỏi tự do — thêm endpoint là việc phía server. Mà mọi tín hiệu cần thiết đã nằm sẵn trong transcript: nhịp nói (chữ/giây), tỷ lệ câu hỏi/câu cảm thán, từ khoá 7 nhóm chủ đề. Suy từ số đo thì tức thì, offline, **0 Vox**, và giải thích được — mỗi gợi ý kèm LÝ DO bằng con số thật ("nói nhanh 4,2 chữ/giây — nhạc chậm sẽ bị lời lấn"), hiện ở tooltip. Transcript quá ngắn thì KHÔNG gợi ý, nói rõ vì sao — thà không gợi ý còn hơn gợi ý bừa. Cùng nguyên tắc `emphasis_points` của V37: đưa ứng viên, không tự quyết. 1585 passed. |
| V87 | Hai ô cookie có trong cấu hình nhưng chưa bao giờ hiện ra giao diện (Phase H, 2026-08-19) | ✅ Xong + 6 test | `Settings.cookies_from_browser`/`cookies_file` có từ V67 và là cách chữa DUY NHẤT khi TikTok chặn (V85), nhưng không nằm trong `FIELDS` → chỉ sửa được bằng cách mở tệp `.env`. Chính tôi cũng chỉ nhầm người dùng "mở Cài đặt → mục Cookie" khi chỗ đó không tồn tại. Thêm hai ô vào thẻ Nâng cao, nhóm mới "Tải video khó", kèm danh sách trình duyệt đúng tên yt-dlp hiểu; sửa lại lời khuyên trỏ đúng đường. Bài học: **có tuỳ chọn trong `Settings` không có nghĩa là người dùng chạm được vào nó.** |
| V85 | TikTok chặn lượt tải — nói cách chữa có thật (Phase H, 2026-08-19, người dùng báo) | ✅ Xong + 1 test | yt-dlp trả `Unexpected response from webpage request` rồi bảo người dùng đi báo lỗi trên GitHub. **Đo thật từ sandbox**: yt-dlp 2026.07.04 (đúng bản đang đóng gói, cũng là bản mới nhất PyPI), bản master, `--impersonate chrome` (curl_cffi), và 3 `api_hostname` khác nhau — TẤT CẢ đều hỏng ở đúng bước "Downloading webpage" ⇒ không phải lỗi bản cũ, mà là TikTok chặn máy tải ẩn danh. Cách chữa CÓ THẬT trong app là mượn cookie trình duyệt (`COOKIES_FROM_BROWSER`, đã có từ V67) — nay được nói thẳng trong `FRIENDLY_ERRORS` kèm phương án 2 (tải bằng trình duyệt rồi dùng nút Tải tệp lên). **Không kiểm chứng được từ đây** vì IP sandbox bị chặn ngay từ bước đầu. |
| V83 | Trình cài đặt tự động CHƯA TỪNG chạy được (Phase H, 2026-08-19, người dùng bấm nút mới của V81 thì lộ) | ✅ Xong + 5 test | `icons.brand_logo()` được gọi ở BA nơi (`icons.app_logo`, `setup_wizard.py`, `app.py`) nhưng **chưa bao giờ được định nghĩa** → dựng `SetupWizard` là ném `AttributeError` ngay dòng logo. Mà `_maybe_first_run` bọc `except Exception` rồi bỏ qua IM LẶNG → trình cài đặt tự động (FFmpeg/Whisper/VieNeu) **chưa chạy được lần nào qua nhiều bản phát hành**, không ai biết. Đây là lý do sâu xa khiến người dùng phải tự cài tay mọi thứ rồi liên tục gặp lỗi. Sửa: viết `brand_logo()` thật (vẽ tay bằng token màu), sửa call site gọi nhầm `.pixmap()` lên QPixmap, và **bỏ nuốt im lặng** — wizard hỏng thì ghi log + báo toast. Test quét AST mọi `icons.<tên>` trong gói giao diện đối chiếu thuộc tính thật. **Bài học: `except Exception` không kèm log = một lỗi có thể sống nhiều tháng.** |
| V81 | "Máy chưa có FFmpeg" sau khi nâng cấp (Phase H, 2026-08-19, người dùng báo bằng ảnh chụp) | ✅ Xong + 7 test | Ba thứ sai cùng lúc, cả ba đều là lỗi của app: (1) trình cài đặt tải FFmpeg về `<app>/bin` nên nâng cấp sang thư mục mới là tệp nằm lại bản cũ — cùng cảnh ngộ `.venv-*` đã sửa ở V77; (2) marker của wizard nằm ở `~/.voxdub_cache`, tức theo MÁY chứ không theo thư mục ứng dụng, nên bản mới tưởng "đã cài xong rồi" và không mời cài lại; (3) hộp thoại chặn chỉ dẫn tải tay từ gyan.dev rồi sửa PATH, **trong khi app có sẵn `FFmpegDownloadWorker` tự tải được** — bắt người dùng làm phần việc của mình. Sửa: mượn `bin/` của bản cũ qua PATH, wizard hiện lại khi thiếu FFmpeg, hộp thoại có nút "Tải giúp tôi" rồi tự kiểm lại điều kiện máy, và lời khuyên đổi sang "chép 2 tệp vào thư mục bin" (ai cũng làm được) thay vì sửa PATH. **Test của chính đợt này làm đỏ 3 test khác**: `_frozen.init()` có `os.chdir()` — gọi trong test mà không trả lại thư mục làm việc thì mọi test dùng đường dẫn tương đối hỏng theo. 1536 passed. |
| V80 | "Cài rồi mà app vẫn bảo chưa" — hai lỗi chặn cứng người dùng (Phase H, 2026-08-19, người dùng báo bằng ảnh chụp) | ✅ Xong + 19 test | **(a) `asr_whisper_worker.py` CHƯA BAO GIỜ nằm trong `datas` của `autodub.spec`.** Nên trên MỌI bản đóng gói từ trước tới nay, `scripts/setup_whisper.py` luôn chết ở `!! không thấy worker script`, `installed_ok.json` không bao giờ được ghi, app mãi báo "chưa cài bộ nghe" — và ngay cả khi có marker thì `_transcribe_whisper_subprocess` cũng không có tệp để chạy. Đây là gốc rễ sâu hơn cả V74/V75: hai mini-spec đó sửa đúng phần "app kiểm sai", nhưng đường chép lời trong `.exe` vốn dĩ chưa từng chạy được. Test viết cho ca này quét `bundled_file()` toàn repo rồi đối chiếu `datas` — **lòi ra tệp thứ hai cũng thiếu: `diarize_worker.py`** (nhận diện người nói). **(b) Bộ cài tạo venv bằng Python 3.14** → `kaldi-native-fbank`/ONNX chưa có wheel → pip build từ nguồn rồi gãy. Tệp .bat đã thử `py -3.12` trước nhưng thủng ở hai cảnh thật: máy có 3.12 mà `py -3.12` không thấy (Python Install Manager `pythoncore-3.14-64`), và venv 3.14 của lần trước khiến mọi lần cài sau đều "venv đã có — bỏ qua". Sửa trong CHÍNH script cài: tự chạy lại bằng bản được hỗ trợ, không có thì dừng sớm với 1 dòng chỉ dẫn; venv cũ tạo bằng bản không hỗ trợ thì xoá dựng lại. 1529 passed. |
| V76 | Nút Dừng dừng được cả lúc canh chữ karaoke (Phase H, 2026-08-19) | ✅ Xong + 8 test | Canh chữ là bước LÂU NHẤT của việc ghi phụ đề nhưng là bước duy nhất trên đường đó không nhìn cờ Dừng: `SubtitleWorker.cancel()` chỉ có tác dụng SAU khi `build_karaoke_ass` xong, còn pipeline thì `rep.check_cancelled()` chỉ chạy GIỮA hai bước. Nối `cancel_event` xuyên `refresh_subtitles → build_karaoke_ass → resolve_word_times → align_segments → worker`, và **giết tiến trình con** thay vì chỉ kiểm cờ ở đầu vòng đọc (bài học V72: kiểm-rồi-chờ chỉ đúng khi cái chờ ngắn). Ba tầng `except Exception` trên đường này phải re-raise `PipelineCancelled`, nếu không cú bấm Dừng biến thành "canh hỏng" rồi chạy tiếp (đúng lỗi V74 với `TranscribeCancelled`). **Lỗi tự soi ra khi đo đối chứng**: dừng lúc ĐANG NẠP MODEL thì `readline` trả rỗng → báo "không phản hồi ready", tầng trên hiểu là hỏng và chạy lại toàn bộ ở in-process. **Số đo**: dừng giữa lúc nạp model 1,0s (bản sửa) so với 20,1s (không có luồng canh). 1462 passed. |
