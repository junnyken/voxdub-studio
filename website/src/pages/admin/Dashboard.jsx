...
/** Bảng cohort retention theo tuần (mini-spec V10, docs/PLAN.md) — dùng lại
 * Device.firstSeenAt/lastSeenAt đã có sẵn, không cần thêm thu thập dữ liệu. */
      ...
      <table className="text-xs w-full">
        <thead>
          <tr className="text-ink-muted">
            <th className="text-left font-normal pr-3 py-1">Tuần đăng ký</th>
            <th className="text-left font-normal pr-3 py-1">Số máy</th>
            {Array.from({ length: maxOffset + 1 }, (_, i) => (
              <th key={i} className="text-center font-normal px-1.5 py-1">Tuần +{i}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {cohorts.map((c) => (
            <tr key={c.cohortWeek} className="border-t border-border-subtle">
              <td className="pr-3 py-1.5 whitespace-nowrap">{c.cohortWeek}</td>
              <td className="pr-3 py-1.5">{c.cohortSize}</td>
              {Array.from({ length: maxOffset + 1 }, (_, i) => {
                const cell = c.retention[i]
                if (!cell) return <td key={i} className="px-1.5 py-1.5" />
                const alpha = Math.max(0.08, cell.pct / 100)
                return (
                  <td key={i} className="px-1.5 py-1.5 text-center">
                    <span
                      className="inline-block rounded px-1.5 py-0.5 min-w-[3rem]"
...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="card p-5">
        <h2 className="font-semibold text-sm">Giữ chân thiết bị theo tuần (retention cohort)</h2>
        <p className="text-xs text-ink-muted mt-1">
          % thiết bị của mỗi tuần đăng ký còn hoạt động ở các tuần sau đó.
        </p>
